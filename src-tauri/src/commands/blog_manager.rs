...
/// returns: 是否需要安装模板
#[tauri::command]
pub async fn init_or_open_blog(path: Utf8PathBuf) -> Result<bool> {
    if !path.exists() {
...
#[tauri::command]
pub async fn install_template(path: Utf8PathBuf, on_progress: OnProgress) -> Result<()> {
...
#[tauri::command]
pub async fn read_schemas(project_dir: Utf8PathBuf) -> Result<HashMap<String, Value>> {
    let path = project_dir.join(TEMPLATE_DIR).join(".astro/collections");
    utils::read_schema_file(path).await
}

#[tauri::command]
pub async fn read_blog_build_config(project_dir: Utf8PathBuf) -> Result<BlogBuildConfig> {
